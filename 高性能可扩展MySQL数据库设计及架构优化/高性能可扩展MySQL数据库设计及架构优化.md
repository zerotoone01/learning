...
**如何解决单点问题：**
	...
	集群中的任何一个服务器宕机后，其他服务器可以取代宕机服务器；
	
MySQL主从复制架构
![img](./img/05_2019-07-07_15-12-35.png)
主从复制步骤：
	1.主库将变更写入到主库的binlog中（一些mysql版本并不会开启二进制日志，一定要检查是否开启，增量备份依赖二进制日志）；
	2.从数据库的IO进程读取主库binlog内容存储到Relay Log日志中（1.二进制日志点，2.GTID（mysql>=5.7版本推荐使用））；
	3.从库的SQL进程读取Relay Log日志中内容在从库中重放；

MySQL主从配置步骤
![img](./img/05_2019-07-07_15-14-15.png)  
...
配置主从数据库服务器参数
	...
	master服务器配置:
		##二进制日志指定存放位置，如果不指定，会存放在/data目录下
		log_bin=/data/mysql/sql_log/mysql-bin
		##服务id，主从服务器各个id不一样最好，涉及到主从切换的时候，如果相同的id会导致失败
		server_id=100
...
在Master服务器上创建用于复制的数据库账号
	...
	需要REPLICATIONSLAVE权限
	CREATE USER`repl`@`IP段` identified by `Password`;
	GRANT REPLICATION SLAVEON *.* TO `repl`@`IP段`;

初始化Slave数据

	建议主从数据库服务器采用相同的MySQL版本
	建议使用全备份的方式初始化Slave数据
	mysqldump --master-data=2 -uroot -p -A --single-transaction -R --triggers

启动基于日志点的复制链路
![img](./img/05_2019-07-07_15-47-30.png)

### 5-2 mysql主从复制演示
创建用户名
![img](./img/05_2019-07-07_16-03-01.png)

### 5-3 基于GTID的复制链路

![img](./img/05_2019-07-07_16-17-11.png)
...
### 5-4 高可用keepalived实例
![img](./img/05_2019-07-07_16-21-29.png)

**虚拟IP**： 就是一个未分配给真实主机的IP，即对外提供服务器的主机除了有一个真实IP外还有一个虚拟IP；
设置虚拟IP有很多种方法，比如脚本，MHA，MMM

本项目采用keeplived来实现
![img](./img/05_2019-07-07_16-26-03.png)

主主复制配置调整
	
	保证只有一个主提供服务
	另外一个提供只读的服务
![img](./img/05_2019-07-07_16-41-24.png)

![img](./img/05_2019-07-07_16-42-52.png)

keeplived配置
![img](./img/05_2019-07-07_16-44-02.png)
...
如何进行读写分离
	...
	由开发人员根据所执行的SQL类型连接不同的服务器
		优点：完全由开发人员控制，实现根据的灵活；
			由程序员直接连接数据库，性能损耗比较少；
		缺点：对于实时性要求比较高的数据，就不适合在从数据库上查询；
			增加了开发工作量以及代码的复杂性；
			人为控制，容易出错；
			
	由数据库中间层完成读写分离
		优点：由中间件根据查询语法分析，自动完成读写分离；
			对程序透明，对于已有程序不用做任何调整；
		缺点：增加了中间层，对查询效率有损耗；
			对于延迟敏感业务无法自动在主库上执行；

数据库中间层完成读写分离
![img](./img/05_2019-07-07_17-17-02.png)
...
lvs架构图
![img](./img/05_2019-07-07_17-25-46.png)
